...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['latestCustomers'] = User::orderBy('id','desc')->whereIsBanned(0)->limit(5)->get();
        $data['currency'] = Currency::whereIsDefault(1)->first();
        $data['bcustomers'] = User::orderBy('id','desc')->whereIsBanned(1)->get();
        $data['todayCustomers'] = User::orderBy('id','desc')->whereIsBanned(0)->whereDate('created_at',today())->get();
        $data['todayDeposits'] = Deposit::where('status','completed')->whereDate('created_at',today())->sum('amount');
        $data['todayInvests'] = Order::where('status','=','completed')->whereDate('created_at',today())->sum('pay_amount');
