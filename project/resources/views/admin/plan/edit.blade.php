    ...
    <div class="card-body">
      ...
      <form class="geniusform" action="{{route('admin.plan.update',$data->id)}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" id="min_price" name="min_price" placeholder="{{ __('Enter Minimum Price') }}" min="1" value="{{$data->min_price}}" required>
          </div>

          <div class="form-group">
            <label for="max_price">{{ __('Maximum Price in') }} ({{$currency->name}})</label>
            <input type="number" class="form-control" id="max_price" name="max_price" placeholder="{{ __('Enter Maximum Price') }}" min="1" value="{{$data->min_price}}" required>
          </div>
